...
function RetroMarquee() {
  return (
    <div className="overflow-hidden py-1" style={{
      ...
      <div className="animate-marquee whitespace-nowrap" style={{ fontFamily: "'VT323', monospace", fontSize: '17px', color: '#000', fontWeight: 'bold' }}>
        ⚡ TIẾT KIỆM LÀ SỨC MẠNHH ⚡ ĐỪNG BỎ CUỘC BRO ⚡ 
        MONEY MONEY MONEYY ⚡ TÍCH TIỂU THÀNH ĐẠI ⚡ 
        CHIẾN BINH TIẾT KIỆM ⚡ GOM TỪNG ĐỒNGG ⚡ 
        LVL UP MỖI NGÀYY ⚡
      </div>
    </div>
  );
...
function LuckyWheel({ onResult, amounts }: { onResult: (amount: number) => void; amounts: number[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <button onClick={spin} disabled={spinning} className="btn-3d-red px-6 py-2" style={{ fontFamily: "'VT323', monospace", fontSize: '22px' }}>
        {spinning ? '⚡ ĐANG QUAY...' : '🎲 QUAY NGAY!'}
      </button>
      {result !== null && (
        <div className="text-center animate-bounce-in" style={{ fontFamily: "'VT323', monospace" }}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.9)' }}>
      ...
      <div className="animate-bounce-in relative max-w-lg w-full p-1" style={{
        ...
        <div className="p-6 text-center" style={{
          background: 'linear-gradient(135deg, #0a0a18, #111122)',
          ...
          <div className="flex justify-center gap-3 my-3">
            {['🔥', '⚡', '💀', '🐉', '⚔️'].map((e, i) => (
              <span key={i} style={{ fontSize: '28px', animation: `float-y2k ${1.5 + i * 0.2}s ease-in-out infinite` }}>{e}</span>
            ))}
          </div>
          ...
          <button onClick={onClose} className="btn-3d-gold mt-4 px-8 py-2" style={{ fontFamily: "'VT323', monospace", fontSize: '20px' }}>
            ⚡ ĐÓNG ⚡
          </button>
        </div>
      </div>
...
function ChallengeProgress({
  ...
  const totalSaved = challenge.completedDays.reduce((sum, done, i) =>
    done ? sum + dayAmounts[i] : sum, 0
  );
  const totalTarget = dayAmounts.reduce((sum, a) => sum + a, 0);
  const isCompleted = completedCount === challenge.totalDays;

  const handleDayClick = (dayIndex: number) => {
    if (challenge.completedDays[dayIndex]) {
      onToggleDay(dayIndex, dayAmounts[dayIndex]);
      return;
    }
    if (template.type === 'random') {
      setWheelDay(dayIndex);
    } else {
      onToggleDay(dayIndex, dayAmounts[dayIndex]);
      playY2KSuccess();
    }
  ...
  useEffect(() => {
    if (isCompleted && !showWin) setShowWin(true);
  }, [isCompleted]);
  ...
  return (
    <div className="min-h-screen grid-pattern relative">
      ...
      {showWin && (
        <WinCertificate challenge={challenge} userName={userName} total={totalSaved} onClose={() => setShowWin(false)} />
      )}
      ...
      <div className="relative z-10 p-3 max-w-5xl mx-auto">
        ...
        <div className="retro-panel p-4 mb-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            <div className="retro-panel-fire p-2 text-center">
              <p style={{ fontFamily: "'VT323', monospace", fontSize: '13px', color: '#ff4400' }}>💰 ĐÃ GOM</p>
            ...
            <div className="retro-panel-cyan p-2 text-center">
              <p style={{ fontFamily: "'VT323', monospace", fontSize: '13px', color: '#00d4ff' }}>🎯 MỤC TIÊU</p>
            ...
            <div className="retro-panel-green p-2 text-center">
              <p style={{ fontFamily: "'VT323', monospace", fontSize: '13px', color: '#39ff14' }}>📊 TIẾN ĐỘ</p>
              <p style={{ fontFamily: "'Press Start 2P', monospace", fontSize: '10px', color: '#fff', lineHeight: '2.2' }}>
          ...
          <div className="relative h-7" style={{ background: '#0a0a12', border: '3px inset #333' }}>
            <div className="h-full progress-stripe transition-all duration-500" style={{
              width: `${progress}%`,
              background: `linear-gradient(90deg, ${template.color}, #ffd700)`,
            }} />
        ...
        {wheelDay !== null && (
          <div className="fixed inset-0 z-40 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.9)' }}>
            <div className="retro-panel p-5 text-center">
              <h3 className="mb-3 animate-electric" style={{ fontFamily: "'Press Start 2P', monospace", fontSize: '11px', lineHeight: '2' }}>
                🎲 VÒNG QUAY SỐ PHẬN 🎲
              </h3>
              ...
              <LuckyWheel
                ...
                amounts={[5000, 10000, 15000, 20000, 25000, 30000, 50000, 2000, 8000, 12000]}
              />
              ...
              return (
                <button key={i} onClick={() => handleDayClick(i)}
                  ...
                  style={{
                    ...
                    boxShadow: done
                      ? `0 0 10px ${template.color}44, inset 0 0 12px ${template.color}22`
                      : '2px 2px 0 #050508',
                    padding: '3px 2px',
                ...
                >
                  <span style={{
                    ...
                    fontSize: challenge.totalDays <= 30 ? '14px' : '11px',
                    color: done ? '#fff' : '#555',
                  }}>
                  ...
                  <span style={{
                    fontFamily: "'VT323', monospace",
                    fontSize: challenge.totalDays <= 30 ? '12px' : '9px',
                    color: done ? '#ffd700' : '#444',
                  }}>
                    {amount >= 1000000 ? (amount / 1000000).toFixed(0) + 'tr' : (amount / 1000).toFixed(0) + 'k'}
                  </span>
                  {done && <span style={{ fontSize: challenge.totalDays <= 30 ? '13px' : '10px' }}>✅</span>}
                </button>
              );
            })}
          </div>
        ...
        {isCompleted && (
          <div className="text-center mt-4">
            <button onClick={() => setShowWin(true)} className="btn-3d-gold px-6 py-3 animate-pulse-glow"
...
export default function App() {
  ...
  const startChallenge = (templateId: string) => {
    ...
    if (existing) {
      ...
      updateData({ ...data, activeChallengeId: templateId });
      return;
    }
    ...
    playY2KClick();
    const newData = { ...data, challenges: [...data.challenges, newChallenge], activeChallengeId: templateId };
    updateData(newData);
    setSelectedChallenge(templateId);
  };
  ...
  const deleteChallenge = (id: string) => {
    ...
    updateData({ ...data, challenges, activeChallengeId: null });
    setSelectedChallenge(null);
  };

  const saveName = () => {
    updateData({ ...data, userName: tempName });
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-3 pb-10">
        ...
        <div className="text-center py-6">
          ...
          <div className="flex justify-center gap-2 mt-2">
            {['🔥', '💰', '⚡', '💰', '🔥'].map((e, i) => (
              <span key={i} style={{ fontSize: '20px', animation: `float-y2k ${2 + i * 0.3}s ease-in-out infinite` }}>{e}</span>
            ))}
          </div>
        </div>
        ...
        <div className="retro-panel p-3 mb-4 flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <p style={{ fontFamily: "'VT323', monospace", fontSize: '20px', color: '#39ff14' }}>
                ⚡ {data.challenges.filter(c => !c.isCompleted).length} đang chiến · {data.challenges.filter(c => c.isCompleted).length} đã GG ✓
              </p>
            </div>
              ...
              {data.challenges.map(ch => {
                ...
                return (
                  <div key={ch.id} className="retro-panel-fire p-3 relative">
                    {ch.isCompleted && <div className="absolute top-1 right-2" style={{ fontSize: '22px' }}>🏆</div>}
                    <div className="flex items-center gap-2 mb-2">
                      <span style={{ fontSize: '22px' }}>{tmpl.emoji}</span>
                      <p style={{
                        fontFamily: "'Press Start 2P', monospace", fontSize: '8px',
                        color: tmpl.color, lineHeight: '1.6',
                      }}>
                        {ch.name}
                      </p>
                    </div>
                    ...
                    <div className="h-4 mb-2" style={{ background: '#0a0a12', border: '2px inset #222' }}>
                      <div className="h-full progress-stripe" style={{
                        width: `${prog}%`, background: `linear-gradient(90deg, ${tmpl.color}, #ffd700)`,
                      }} />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => { playY2KClick(); setSelectedChallenge(ch.id); updateData({ ...data, activeChallengeId: ch.id }); }}
                        className="btn-3d-blue flex-1 py-1" style={{ fontFamily: "'VT323', monospace", fontSize: '18px' }}>
                        ▸ CHIẾN TIẾPP
                      </button>
        ...
        <div>
          ...
          <div className="grid gap-3 grid-cols-1 sm:grid-cols-2">
            {CHALLENGES.map((tmpl) => {
              ...
              return (
                <div key={tmpl.id} className="retro-panel-cyan p-4 relative overflow-hidden"
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start gap-3 mb-2">
                      <div className="flex-shrink-0 text-center">
                        <span style={{ fontSize: '28px', display: 'block' }}>{tmpl.emoji}</span>
                        <Y2KIcon src={tmpl.icon} size={32} className="animate-float mt-1" />
                      </div>
                      <div className="min-w-0">
                        ...
                        <p className="mt-1" style={{
                          fontFamily: "'VT323', monospace", fontSize: '17px', color: '#999', lineHeight: '1.3',
                        }}>
                          {tmpl.desc}
                        </p>
                      </div>
                    ...
                    <div className="flex gap-1 mb-3 flex-wrap">
                      ...
                      {days > 5 && (
                        <span style={{ fontFamily: "'VT323', monospace", fontSize: '13px', color: '#444' }}>
                          ...+{days - 5}
                        </span>
                      )}
                    </div>
                    ...
                    <button onClick={() => startChallenge(tmpl.id)}
                      className={exists ? 'btn-3d-gold w-full py-2' : 'btn-3d-red w-full py-2'}
        ...
        <div className="mt-8 text-center">
          <div className="flex justify-center gap-3 mb-2">
            {['🔥', '⚡', '💀', '🐉', '⚔️'].map((e, i) => (
              <span key={i} style={{ fontSize: '18px', opacity: 0.5 }}>{e}</span>
            ))}
          </div>
          <p style={{ fontFamily: "'VT323', monospace", fontSize: '16px', color: '#444' }}>
            ⚡ Chiến Binh Tiết Kiệm ⚡ Made for the boys 💪 ⚡
          </p>
